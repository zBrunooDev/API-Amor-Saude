...
@router.get("/consultas?pacienteId={paciente_id}")
def listar_consultas_por_paciente(paciente_id: str, db: Session = Depends(get_db)):

    consultas = crud.get_consultas_by_paciente_id(db, paciente_id)

    return consultas


@router.get("/consultas/{consulta_id}")
def buscar_consulta(consulta_id: str, db: Session = Depends(get_db)):

    consulta = crud.get_consulta_by_id(db, consulta_id)
...
def deletar_consulta(consulta_id: str, db: Session = Depends(get_db)):
    ...
    deleted_consulta = crud.delete_consulta(db, consulta_id)
